...
namespace SWDirectCollect_Channel8
{
    public partial class UI_RFLocalCtrl : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        {
            int ret = 0;
            if (simpleButton5.Text == "窄带AGC开启")
            {
                simpleButton5.Text = "窄带AGC关闭";
                ret = DLLImport.NBDC_AGC(1);
                //this.m_DataProcessing.DigitialGain_24db = true;
            }
            else if (simpleButton5.Text == "窄带AGC关闭")
            {
                simpleButton5.Text = "窄带AGC开启";
                ret = DLLImport.NBDC_AGC(0);
                //this.m_DataProcessing.DigitialGain_24db = false;
            }
            if (passCtrlBack != null)
            {

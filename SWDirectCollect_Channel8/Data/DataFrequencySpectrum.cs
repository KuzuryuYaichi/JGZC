    ...
    public class DataFrequencySpectrum
    {
        ...
        void m_queue_TEvent(IQDataAndFreq t)
        {
            ...
            else
            {
                ...
                if (t.Type == 2)
                    offset = 125.16 + Resolution_Offset;
                else if(t.Type == 1)
                    offset = 125.16 + Resolution_Offset + (m_DataProcessing.DigitialGain_24db ? 24 : 0) + (m_DataProcessing.LowNoise ? 12 : 0);
            }
            ...
            else
            {
                ...
                for (int i = 0; i < outData.Length / 2; i++)
                {
                    ...
                    for (int j = 0; j < mm_SmoothNum; j++)
                        sum += IQdata[i][j];
                    sum /= mm_SmoothNum;
                    m_DigitalGain = Convert.ToInt32(t.Digital_Gain[0]);
                    m_RFGain = Convert.ToInt32(t.RF_Gain[0]);
                    power[i] = Math.Log10(sum) * 20 - offset + m_DigitalGain + m_RFGain;
                }
            }

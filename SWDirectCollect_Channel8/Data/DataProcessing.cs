...
namespace SWDirectCollect_Channel8
{
    public class DataProcessing
    {
        ...
        public int udpRecvInit(string LocalIP, string[] GroupIP, int GroupPort)//窄带宽带数据接收与处理
        {
            //if (udpRecv != null)
            //{
            //    udpRecv.udpInit(LocalIP, GroupIP, GroupPort);
            //}
            //else
            //{
                udpRecv = new udpRecv(LocalIP, GroupIP, GroupPort);//建立udp连接
                udpRecv.passParameter += new SWDirectCollect_Channel8.udpRecv.PassInformationr(udpRecv_passParameter);//委托
            //}
        ...
        /// 选择是否进行性能测试
        private bool m_FlagTesting = false;
        public void SelectedFlagTesting(bool FlagTesting)
        {
            m_FlagTesting = FlagTesting;
        }
        ...
        #region 初始化
        public Test_DataProcessing Test_DataProcessing;
        ...
        public DataProcessing()
        {
            ...
            Test_DataProcessing = new Test_DataProcessing();//（TCP）
        }
        ...
        FFT_Data DataProcessFFT(byte[] t)
        {
            ...
            int key = BitConverter.ToInt32(t, 3);
        ...
        void udpRecv_passParameter(udpRecv sender, byte[] t)//委托事件，udpRecv类传递的数据LX
        {
            ...
            switch (Header_DataType)
            {
                ...
                case 1://200305LXtemp
                        ...
                        nDataAndFreq.RF_Gain = new int[m_ChannelCount];//定义射频增益
                        ...
                        nDataAndFreq.Digital_Gain = new int[1];//定义数字增益
                        //for (int i = 30; i <= 54; i++)
                        //{
                        //    for (int j = 0; j < 8; j++)
                        //    {
                        //        if((t[i] & (1 << j)) != 0)
                        //        {
                        //            nDataAndFreq.Digital_Gain[m_ChannelCount] = -24;
                        //            break;
                        //        }
                        //    }
                        //}
                        ...
                        if(t[2] >= 0 && t[2] <= 195)
                        {
                            nDataAndFreq.Digital_Gain[0] = ((t[30 + t[2] / 8] & (1 << (t[2] % 8))) != 0) ? -24 : 0;
                        }
        ...
        void m_queue_TEvent(DataAndFreq t)
        {
            ...
            else
            {
                ...
                //{
                //    IQDataResolutionOrg[BaseIndex + 2 * i] = (double)BitConverter.ToInt16(t.Data, 4 * i + 2);//每两个字节IQ数据转换为一个16位有符号整数的数组LX
                //    IQDataResolutionOrg[BaseIndex + 2 * i + 1] = (double)BitConverter.ToInt16(t.Data, 4 * i);//每两个字节IQ数据转换为一个16位有符号整数的数组LX
                //}
            ...
            if (m_FlagTesting == true)
            {
                Test_DataProcessing.PushData(t);
            }
        }
        #endregion
    }

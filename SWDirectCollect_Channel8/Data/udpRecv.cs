...
namespace SWDirectCollect_Channel8
{
    public class TimeInfo
    {
        ...
        public class BD_GPS
        {
            ...
            public BD_GPS(byte[] t)
            {
                ...
                {
                    DateTime dt = new DateTime(year, month, day, hour, minute, second).AddSeconds(1);
                    this.year = dt.Year;
                    this.month = dt.Month;
                    this.day = dt.Day;
                    this.hour = dt.Hour;
        ...
        public TimeInfo(byte[] t)
        {
            ...
            int nanosecond = (int)(BitConverter.ToUInt32(t, 11) / 192);
            ...
            {
                //DateTime dt;
                //if (t[1] == 2 && (millisecond + 819) >= 1000)
                //    dt = new DateTime(year, month, day_offset, hour, minute, second).AddSeconds(2);
                //else
                //    dt = new DateTime(year, month, day_offset, hour, minute, second).AddSeconds(1);
                DateTime dt = new DateTime(year, month, day_offset, hour, minute, second).AddSeconds(1);
                this.year = dt.Year;
                this.month = dt.Month;
    ...
    public class udpRecv
    {
        ...
        public void udpInit(string LocalIP, string GroupIP, int GroupPort)
        {
            ...
            //加入组播组
            m_RecvSocket.SetSocketOption(SocketOptionLevel.IP, SocketOptionName.AddMembership, new MulticastOption(IPAddress.Parse(GroupIP)));
        ...
        {
            #region 丢包计数
            UInt32 curIndex = BitConverter.ToUInt32(t, 3);
            if (RevCount == 0)
            {
